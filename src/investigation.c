...
static int valid_id(const char *s, const char *prefix)
{
    ...
    n = strlen(prefix);
    if (_strnicmp(s, prefix, n) != 0 || s[n] == '\0') return 0;
    for (i = n; s[i]; ++i)
...
static int load_record(const char *id, investigation_record_t *record)
{
    ...
    if (fopen_s(&file, RECORD_PATH, "r") != 0 || file == NULL) { free(line); return 0; }
    while (fgets(line, LINE_MAXIMUM, file)) {
        char *fields[8], *context = NULL, *token;
        size_t count = 0;
        line[strcspn(line, "\r\n")] = '\0';
        token = strtok_s(line, "\t", &context);
        while (token && count < 8) { fields[count++] = token; token = strtok_s(NULL, "\t", &context); }
        if ((count == 7 || count == 8) && _stricmp(fields[0], id) == 0) {
...
{ sprintf_s(output, 32, "CAN-%08lX", id_hash(intelligence_id)); }
    ...
    const char *artifact, char output[65])
{
    ...
    if (fopen_s(&history, history_path, "a") != 0 || !history) return 0;
...
{
    FILE *file = NULL;
    if (fopen_s(&file, INDEX_PATH, "a") != 0 || !file) return 0;
...
static int index_activate(const char *candidate)
{
    ...
    sprintf_s(temp, sizeof(temp), "%s.tmp", INDEX_PATH);
    if (fopen_s(&in, INDEX_PATH, "r") != 0 || !in) return 0;
...
int rictus_investigation_watch_start(const char *candidate)
{
    ...
    paths_for(candidate, artifact, history); timestamp_now(timestamp); sprintf_s(event, sizeof(event), "SE-%s-WATCH", candidate + 4);
    if (fopen_s(&file, artifact, "ab") != 0 || !file) return RICTUS_INVESTIGATION_IO_ERROR;
...
static int evidence_append_typed(const char *candidate,const char *source,const char *intel_id,const char *rule,evidence_relationship_t relationship,const char *evidence_text)
{
    ...
    paths_for(candidate, artifact, history); timestamp_now(timestamp); sprintf_s(event, sizeof(event), "SE-%s-%08lX", candidate + 4, id_hash(evidence_text));
    if (fopen_s(&file, artifact, "ab") != 0 || !file) return RICTUS_INVESTIGATION_IO_ERROR;
    if (fprintf(file, "\n### %s\n\n- Date/time: %s\n- Evidence source: %s\n- Relationship: %s\n"
...
static int contains_ci(const char *s,const char *n){size_t z;const char *p;if(!s||!n||!*n)return 0;z=strlen(n);for(p=s;*p;++p)if(_strnicmp(p,n,z)==0)return 1;return 0;}
static int extract_cve(const char *s,char out[32]){const char *p;size_t n=0;if(!s)return 0;for(p=s;*p;++p)if(_strnicmp(p,"CVE-",4)==0)break;if(!*p)return 0;while(p[n]&&n<31&&((p[n]>='A'&&p[n]<='Z')||(p[n]>='a'&&p[n]<='z')||(p[n]>='0'&&p[n]<='9')||p[n]=='-')){out[n]=p[n];++n;}out[n]='\0';return n>4;}
static int correlate(const investigation_record_t *origin,const investigation_record_t *item,char rule[96])
{
    ...
    if(extract_cve(origin->title,a)||extract_cve(origin->summary,a)||extract_cve(origin->content,a))if((extract_cve(item->title,b)||extract_cve(item->summary,b)||extract_cve(item->content,b))&&_stricmp(a,b)==0){sprintf_s(rule,96,"shared CVE %s",a);return 1;}
    for(i=0;i<2;++i){const char *path=i?"/app/core/config.php":"/app/core/mailer.php";if((contains_ci(origin->summary,path)||contains_ci(origin->content,path))&&(contains_ci(item->summary,path)||contains_ci(item->content,path))){sprintf_s(rule,96,"shared protected path %s",path);return 1;}}
    for(i=0;i<_countof(products);++i)if((contains_ci(origin->title,products[i])||contains_ci(origin->summary,products[i]))&&(contains_ci(item->title,products[i])||contains_ci(item->summary,products[i])))for(j=0;j<_countof(classes);++j)if((contains_ci(origin->title,classes[j])||contains_ci(origin->summary,classes[j]))&&(contains_ci(item->title,classes[j])||contains_ci(item->summary,classes[j]))){sprintf_s(rule,96,"shared product %s and class %s",products[i],classes[j]);return 1;}
    return 0;
}
...
{
    FILE *f=NULL;char *line;unsigned long row=0,cursor=cursor_get(candidate);investigation_record_t origin,item;if(!load_record(intel_id,&origin))return;line=(char*)malloc(LINE_MAXIMUM);if(!line)return;if(fopen_s(&f,RECORD_PATH,"r")!=0||!f){free(line);return;}
    while(fgets(line,LINE_MAXIMUM,f)){char *fields[8],*ctx=NULL,*tok,rule[96],evidence[1200];size_t count=0;++row;if(row<=cursor)continue;line[strcspn(line,"\r\n")]='\0';tok=strtok_s(line,"\t",&ctx);while(tok&&count<8){fields[count++]=tok;tok=strtok_s(NULL,"\t",&ctx);}if(count<7||_stricmp(fields[0],intel_id)==0)continue;memset(&item,0,sizeof(item));strcpy_s(item.id,sizeof(item.id),fields[0]);unescape(fields[1],item.source,sizeof(item.source));unescape(fields[2],item.title,sizeof(item.title));unescape(fields[5],item.summary,sizeof(item.summary));if(count==8)unescape(fields[6],item.content,sizeof(item.content));if(correlate(&origin,&item,rule)){sprintf_s(evidence,sizeof(evidence),"%s | %s | %s",item.id,item.title,item.summary);if(evidence_append_typed(candidate,item.source,item.id,rule,REL_SUPPORTING,evidence)==RICTUS_INVESTIGATION_OK)notice_queue(candidate,item.id,rule);}}
...
static rictus_module_result_t command_candidate(const rictus_module_command_t *command, rictus_module_command_reply_fn reply, void *context, void *unused)
{
    ...
    if (result == RICTUS_INVESTIGATION_OK) { candidate_id_for(command->arguments, candidate); sprintf_s(response, sizeof(response), "CANDIDATE CREATED | %s | SOURCE %s", candidate, command->arguments); }
    else if (result == RICTUS_INVESTIGATION_ALREADY_EXISTS) { index_find(command->arguments, candidate, NULL); sprintf_s(response, sizeof(response), "CANDIDATE ALREADY EXISTS | %s | SOURCE %s", candidate, command->arguments); }
    else if (result == RICTUS_INVESTIGATION_NOT_FOUND) sprintf_s(response, sizeof(response), "CANDIDATE REFUSED | %s | INT NOT FOUND", command->arguments);
    else sprintf_s(response, sizeof(response), "CANDIDATE FAILED | %s | RESULT %d", command->arguments, result);
...
static rictus_module_result_t command_watch(const rictus_module_command_t *command, rictus_module_command_reply_fn reply, void *context, void *unused)
{
    ...
    if (result == RICTUS_INVESTIGATION_OK) sprintf_s(response, sizeof(response), "WATCH ACTIVE | %s", command->arguments);
    else if (result == RICTUS_INVESTIGATION_ALREADY_EXISTS) sprintf_s(response, sizeof(response), "WATCH ALREADY ACTIVE | %s", command->arguments);
    else if (result == RICTUS_INVESTIGATION_NOT_FOUND) sprintf_s(response, sizeof(response), "WATCH REFUSED | %s | CANDIDATE NOT FOUND", command->arguments);
    else sprintf_s(response, sizeof(response), "WATCH FAILED | %s | RESULT %d", command->arguments, result);
    return reply(context, response) ? RICTUS_MODULE_OK : RICTUS_MODULE_ERR_START_FAILED;
}
...
static rictus_module_result_t command_inv(const rictus_module_command_t *command, rictus_module_command_reply_fn reply, void *context, void *unused)
{
    ...
    if(_stricmp(command->arguments,"scan")==0){scan_active_watches();return reply(context,"Investigation scan complete | explainable relationships appended | lifecycle state unchanged")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
    ...
    if (fopen_s(&file, INDEX_PATH, "r") == 0 && file) {
        while (fgets(line, sizeof(line), file)) { ++candidates; if (strstr(line, "\tACTIVE") != NULL) ++watches; }
        fclose(file);
    }
    sprintf_s(response, sizeof(response), "Investigation %u.%u.%u | ABI %u.%u | state=%s",
    ...
    sprintf_s(response, sizeof(response), "Candidates=%u | active watches=%u | correlation worker=%s | mode=SHADOW", candidates, watches,g_worker?"ACTIVE":"INACTIVE");
    return reply(context, response) ? RICTUS_MODULE_OK : RICTUS_MODULE_ERR_START_FAILED;
}
...
static rictus_module_result_t command_im(const rictus_module_command_t *command,rictus_module_command_reply_fn reply,void *context,void *unused)
{
    ...
    if(!command||!reply)return RICTUS_MODULE_ERR_INVALID_ARGUMENT;
    if(_stricmp(command->arguments,"status")==0){rictus_im_metrics(&metrics);sprintf_s(response,sizeof(response),"IM production | drafts=%u review pass=%u review fail=%u approvals=%u feedback=%u",metrics.drafts,metrics.reviews_passed,metrics.reviews_failed,metrics.approvals,metrics.feedback_items);if(!reply(context,response))return RICTUS_MODULE_ERR_START_FAILED;return reply(context,"Publication=HUMAN CONTROLLED | automatic publication/SRT/Chain/RAG/remediation=PROHIBITED")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
    if(_strnicmp(command->arguments,"draft ",6)==0){argument=command->arguments+6;if(rictus_im_draft(argument,command->sender,id,response,sizeof(response))){if(!reply(context,response))return RICTUS_MODULE_ERR_START_FAILED;sprintf_s(response,sizeof(response),"IM DRAFT CREATED | %s | !im review %s",id,id);return reply(context,response)?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}return reply(context,response)?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
    if(_strnicmp(command->arguments,"show ",5)==0){if(!rictus_im_show(command->arguments+5,response,sizeof(response),detail,sizeof(detail)))return reply(context,"IM draft not found.")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;if(!reply(context,response))return RICTUS_MODULE_ERR_START_FAILED;return reply(context,detail)?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
    if(_strnicmp(command->arguments,"review ",7)==0){if(!rictus_im_review(command->arguments+7,response,sizeof(response)))return reply(context,"IM review failed or draft not found.")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;return reply(context,response)?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
    if(_strnicmp(command->arguments,"approve ",8)==0){if(_stricmp(command->sender,"STN_Boss")!=0&&_stricmp(command->account,"STN_Boss")!=0)return reply(context,"IM approval refused: direct STN_Boss role authority required.")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;if(!rictus_im_approve(command->arguments+8,command->sender,response,sizeof(response)))return reply(context,response[0]?response:"IM approval failed or draft not found.")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;return reply(context,response)?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
    if(_strnicmp(command->arguments,"feedback ",9)==0){strcpy_s(detail,sizeof(detail),command->arguments+9);space=strchr(detail,' ');if(!space)return reply(context,"Usage: !im feedback IM-DRAFT-XXXXXXXX <feedback>")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;*space++='\0';if(!rictus_im_feedback(detail,command->sender,space,response,sizeof(response)))return reply(context,"IM feedback failed.")?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;return reply(context,response)?RICTUS_MODULE_OK:RICTUS_MODULE_ERR_START_FAILED;}
...
#define TEST(x) do { ++executed; if (x) ++passed; else ++failed; } while (0)
    ...
    {investigation_record_t a,b;char rule[96];memset(&a,0,sizeof(a));memset(&b,0,sizeof(b));strcpy_s(a.title,sizeof(a.title),"CVE-2026-1234 Drupal access control");strcpy_s(b.summary,sizeof(b.summary),"Independent CVE-2026-1234 report");TEST(correlate(&a,&b,rule)&&strstr(rule,"shared CVE")!=NULL);memset(&b,0,sizeof(b));strcpy_s(b.title,sizeof(b.title),"Unrelated satellite launch");TEST(!correlate(&a,&b,rule));}
    TEST(_stricmp(relationship_name(REL_CONTRADICTORY),"CONTRADICTORY")==0);TEST(RICTUS_INVESTIGATION_VERSION_MINOR==3);
